...
template "#{node['cloudera_manager']['nginx']['cloudera_manager']['sites_available_dir']}/#{node['cloudera_manager']['nginx']['cloudera_manager']['site_name']}-#{node['hostname']}" do
  ...
  variables({
      ...
      :ssl_certificate => "#{node['cloudera_manager']['nginx']['cloudera_manager']['ssl_dir']}/#{node['cloudera_manager']['nginx']['cloudera_manager']['ssl_crt_file']}",
      :ssl_certificate_key => "#{node['cloudera_manager']['nginx']['cloudera_manager']['ssl_dir']}/#{node['cloudera_manager']['nginx']['cloudera_manager']['ssl_key_file']}"
            })
  action :create
...
link "/etc/nginx/sites-enabled/#{node['cloudera_manager']['nginx']['cloudera_manager']['site_name']}-#{node['hostname']}" do
  to "/etc/nginx/sites-available/#{node['cloudera_manager']['nginx']['cloudera_manager']['site_name']}-#{node['hostname']}"
  notifies :restart, "service[nginx]"
end
